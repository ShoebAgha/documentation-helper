...
async def index_document_async(documents: List[Document], batch_size: int =50):
    ...
    log_info(
        f"VectorStore Indexing: Split into {len(batches)} batches of {batch_size} documents each", Colors.DARKCYAN
    )
...
async def main():
    ...
    res = tavily_crawl.invoke(
        {
            ...
            #'instructions': 'content on ai pages'
        }
    )
    ...
    log_header("DOCUMENT CHUNKING FACE")
    log_info(f"TEXT SPLITTER: Processing {len(all_docs)} documents of chunk size 4000 and overlap of 200", Colors.YELLOW)
    ...
    #log_info(f"   • URLs mapped: {len(site)}")
